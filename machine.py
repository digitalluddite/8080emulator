...
from utils import get_signed_int, to_signed_binary
...
class Machine8080:
    ...
    def rar(self, *args):
        ...
        cy = self._flags[Flags.CARRY]
        A = self._registers[Registers.A]
        logging.info("A & 0x01 = {0}".format(A&0x01))
    ...
    def cmp(self, opcode, *args):
        ...
        if reg == Registers.M:
            ...
            val = self._registers[reg]
        val = get_signed_int(val)
        A = get_signed_int(self._registers[Registers.A])
        ...
        b = to_signed_binary(A)
        logging.info(f'A = {A} {b:02X}')
